...
% Modified Oct 15 for use with data that had no accompanying phantom scan.
% This code DOES NOT calculate TMD.
...
%% Input parameters and check for common errors

% Check if the user is using the correct code
code = input ('\n Do you have a phantom scan for this data set? (Enter "y" or "n") \n\n', 's');
while code ~= 'y' && code ~= 'n'
    code = input ('\n Please enter "y" or "n". \n\n', 's');
end
if code == 'y'
    fprintf('\n This is the wrong code for your data. Use CTgeom_fc instead.\n\n')
    return
elseif code == 'n'
    fprintf('\n Good, this is the correct code for data without a phantom scan.\n')
end
...
%                 tmd_HA = (tmd_ac - eq_num(m,1))/eq_denom(m,1);
                ...
                geometry = [total_cs_area marrow_area cortical_area cort_thickness ...
                    ...
                    Iml section_mod medial_extreme anterior_extreme];
                geom_out(j,:) = geometry;
    ...
    if exist('k', 'var') && exist('folder', 'var') && exist('j', 'var')
        ...
        if k+offset>1 % only attempt to save data if this is not the first sample analyzed
            ...
            headers = {'Total CSA (mm^2)', 'Marrow Area (mm^2)', 'Cortical Area(mm^2)', 'Cortical Thickness (mm)', 'Periosteal BS (mm)', 'Endocortical BS (mm)',  'Imax (mm^4)', 'Theta max (deg)', 'Imin (mm^4)','Theta min (deg)', 'AP Width (mm)', 'ML Width (mm)', 'AP/ML',  'Iap (mm^4)', 'Iml (mm^4)',   'Section Modulus (mm^3)', 'Medial Extreme (mm)', 'Anterior Extreme (mm)'};
